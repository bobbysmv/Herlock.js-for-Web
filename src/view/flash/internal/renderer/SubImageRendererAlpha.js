...
//        +"   color.rgb /= ( color.a + 0.000001 );\n"
        ...
        +"	 gl_FragColor = color;\n"
        //  original
//        +"	gl_FragColor = texture2D( u_texUnitId, v_texCoords ) * v_alpha;\n"//パフォーマンス的に仕方ない?
        +"}"

    var SubImageRendererAlpha = Class( Renderer, function( cls, parent ){
...
//            ShaderManager::getInstance()->put("SubImageRenderer.Shader", shader );
            ...
            this._a_alpha = this._shader.getAttributeLocation( "a_alpha" );
            gl.enableVertexAttribArray( this._a_alpha );
        ...
        cls.render = function() {
            ...
            for( var i = 0; i < numOfObjects; i++ ) {
                ...
                // alpha
                Array.prototype.push.apply( alphaBuffer, [alpha,alpha,alpha,alpha,alpha,alpha] );
            }
            ...
            // vbo TODO alpha
            if( !this._alphaVBO ) this._alphaVBO = gl.createBuffer()
            gl.bindBuffer( gl.ARRAY_BUFFER, this._alphaVBO );
            gl.bufferData( gl.ARRAY_BUFFER, new Float32Array(alphaBuffer), gl.STATIC_DRAW);
            gl.vertexAttribPointer( this._a_alpha, 1, gl.FLOAT, false, 4 * 1, 0 ); checkGlError("glVertexAttribPointer");
